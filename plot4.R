...
with(data,lines(as_date,as.numeric(Global_active_power)))

with(data,plot(as_date, as.numeric(Voltage), type="n", ylab = "Voltage", xlab = "datetime"))
with(data,lines(as_date,as.numeric(Voltage)))
...
legend("topright", c('Sub_metering_1','Sub_metering_2','Sub_metering_3'),lty = c(1, 1, 1), col = c('black','red','blue'),box.col = "transparent" )

with(data,plot(as_date, as.numeric(Global_reactive_power), type="n", ylab = "Global_reactive_power",xlab = "datetime"))
with(data,lines(as_date,as.numeric(Global_reactive_power)))
